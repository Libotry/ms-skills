# RegBase 路线检查规则(950PR 核心)

> **来源**:CANNBot-Skills `ops/ascendc-code-review/references/regbase-review-checks.md`
> **官方条款**:RB-1 ~ RB-5(共 5 条)
> **适用架构**:arch35 及以上(950PR / 950DT)
> **侧别**:Kernel
> **默认启用**:是

RegBase 是 950PR 新增的**基于寄存器的编程模型**,与传统的 MemBase(基于 UB 的 LocalTensor 计算)不同。RegBase 使用 `RegTensor` 在寄存器上直接计算,通过 VF function(`__simd_vf__`)和 `AscendC::Reg::*` API 实现高性能。
...
3. 交叉比对:
   ...
   - 代码中出现两条路线的信号且无明确隔离 → FAIL

**错误示例**:DESIGN.md 声明使用 RegBase 路线,但 `op_kernel/xxx.cpp` 中同时出现 `RegTensor` 和原生 `DataCopy`(非 RegBase 封装),路线混用。

**正确示例**:DESIGN.md 声明 RegBase,代码中所有数据搬运通过 RegBase 封装的 API 完成,无原生 MemBase API 调用。

> ⚠️ **例外**:RegBase outer shell 可使用 `TPipe`、`TQue`、`LocalTensor` 做 UB staging。关键在于 **Compute 的数学核心是否进入寄存器级 VF body**。
...
- mask 参数位置和顺序靠猜
- 模板参数缺失导致 dtype route 不匹配

---

## [RB-3] 寄存器级计算边界(mask / tail)
...
4. Grep `repeatTimes`、`mask`、`tail` 关键词
...
- `regbase-api.md` — RegBase API 白名单与签名
- `regbase-traps.md` — RegBase 常见陷阱
- `regbase-vs-membase.md` — RegBase 与 MemBase 路径混淆
- 原始来源:`ops/ascendc-code-review/references/regbase-review-checks.md`
